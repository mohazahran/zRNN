...
class zLSTM(object):
    ...
    classdocs
    '''
    def __init__(self, inputDim=10, hiddenDim=10, learningRate = 0.01, clipGradients = False, useAdaGrad = False, batchSize = 1):
        ...
        bf = np.random.uniform(-0.1, 0.1, (hiddenDim,1)) #better to be initialized to all ones.
    ...
    def forwardPass(self, inputSeq): #inputSeq is a sequence of input vectors (e.g. an input sentence)
        ...
        H[-1] = np.zeros((self.hiddenDim,1)) # initial hidden state
        C[-1] = np.zeros((self.hiddenDim,1)) # initial hidden state
    ...
    def backProp(self, inputBatch, trueOutputBatch):
        ...
        for b in range(len(inputBatch)): #for each input sentence in the batch
            ...
            for t in reversed(range(T)): #from idx T-1 down to 0
                if t not in dH:
                    dH[t] = np.zeros((self.hiddenDim, 1))
                if t not in dO:
                    dO[t] = np.zeros((self.hiddenDim, 1))
                if t not in dC:
                    dC[t] = np.zeros((self.hiddenDim, 1))
                if t not in dF:
                    dF[t] = np.zeros((self.hiddenDim, 1))
                if t not in dI:
                    dI[t] = np.zeros((self.hiddenDim, 1))
                if t not in dZ:
                    dZ[t] = np.zeros((self.hiddenDim, 1))
                
                xt = np.zeros((self.inputDim,1))
                xt[X[t]][0] = 1.0
                    
                yt = np.zeros((self.inputDim,1))
                yt[trueOutputBatch[b][t]][0] = 1.0
    
                dH[t] += softmaxPredictions[t] - yt
                if(t+1 < T):
...
def generateText(lstm, w2i, i2w, startWordSeed, wordCount):
    ...
    while wordCount > 0:
        ...
        genWordId = softmaxPredictions.T[0].argmax()
...
def main():
    ...
    lstm = zLSTM(inputDim=D, hiddenDim=H, learningRate=0.1, clipGradients= True, useAdaGrad=True, batchSize = 1)
    ...
    for i in range(epochs):
        ...
        if i % 100 == 0:
            ...
            print 'Generated chars:\n', generateText(lstm, w2i, i2w, 'l', 50)
