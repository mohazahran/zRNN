...
LOGFILE = open('statesLog/C.txt', 'w')
...
def generateText(lstm, w2i, i2w, startWordSeed, wordCount):
    if startWordSeed is None: 
        startWordSeed = random.choice(w2i.keys())
    ...
    cntr = 0
    C_mat = np.zeros((wordCount, lstm.hiddenDim))
    while wordCount > 0:
    ...
    #dumping the states
    '''
    C_2d = pca.fit(C_mat).transform(C_mat)
    for i in range(len(C_2d)):
        LOGFILE.write(str(-1)+':'+str(i)+':')
        arr = [float(j) for j in C_2d[i]]
        LOGFILE.write(str(arr))
        LOGFILE.write('\n')
        LOGFILE.flush()
        cntr += 1
    '''
...
def simulateData():
    ...
    w2i = lst[1]
    i2w = lst[2]
    sent = generateText(lstm, w2i, i2w, None, 1310)
    print sent
    ...
    def backProp(self, inputBatch, trueOutputBatch):
        ...
        for b in range(len(inputBatch)): #for each input sentence in the batch
            ...
            for chnk in range(chunckCount):
                ...
                #dumping the states
                if self.epoch % 100 == 0:
                    C_mat = np.zeros((len(C)-1, C[0].shape[0]))
                    for i in range(len(C)-1):
                        C_mat[i] = C[i][0]
                    C_2d = pca.fit(C_mat).transform(C_mat)
                    for i in range(len(C_2d)):
                        LOGFILE.write(str(self.epoch)+':'+str(i)+':')
                        arr = [float(j) for j in C_2d[i]]
                        LOGFILE.write(str(arr))
                        LOGFILE.write('\n')
                    LOGFILE.flush()
